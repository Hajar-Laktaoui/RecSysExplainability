...
class SasrecItemEmbeddings(nn.Module):
    def __init__(self, sasrec_hidden_untis, emsize):
        ...
        path_to_trained_sasrec = '/content/drive/MyDrive/SASRec.pytorch-master/TripAdvisorGP_default/SASRec.epoch=80.lr=0.001.layer=2.head=1.hidden=50.maxlen=200.pth'
...
class SasrecUserEmbeddings(nn.Module):
    def __init__(self, sasrec_hidden_untis, emsize):
        ...
        path_to_trained_sasrec = '/content/drive/MyDrive/SFTmodel/UserEmbTripAdvisor.pt'
    ...
    def forward(self, user_embeds):
        ...
        return user_embeds
'''    
class SasrecRatings(nn.Module):
    def __init__(self, emsize):#def __init__(self, emsize):
        super().__init__()

        # Load the trained data containing ratings
        path_to_trained_sasrec = '/content/drive/MyDrive/SFTmodel/UserEmbTripAdvisor.pt'
        ...
        self.ratings = loaded_data['ratings']  # Shape: (user_number, item_number)
        # self.sentiments = [['positive' if rating == 1 else 'negative' for rating in user_ratings] 
        #                    for user_ratings in self.ratings]
        # Find unique ratings to determine the vocabulary size
        unique_ratings = torch.unique(self.ratings)
        self.vocab_size = unique_ratings.size(0)  # Vocabulary size (number of unique ratings)

        # # Now use this vocab size to create the rating embeddings
        self.rating_embeds = nn.Embedding(self.vocab_size, emsize)

    def forward(self, rating_ids):
        # Correct attribute is self.rating_embeds
        rating_ids = rating_ids.to(self.rating_embeds.weight.device)  # Move rating_ids to the same device as the embedding weights
        rating_embeds = self.rating_embeds(rating_ids.long())
        return rating_embeds
'''
class UIPrompt:
    @classmethod
    ...
    def forward(self, user, item, text, feat, mask, ignore_index=-100):
        ...
        src = torch.cat([u_src1.unsqueeze(1), u_src2.unsqueeze(1), i_src1.unsqueeze(1), i_src2.unsqueeze(1), w_src], 1) #unsqueeze(1) adding new dim, context_src
        if mask is None:
            return super().forward(inputs_embeds=src)
        else:
            # training
            # input padding
            pad_left = torch.ones((batch_size, self.src_len ), dtype=torch.int64).to(device) # concept_embeds.size(1) + feat_embeds.size(1)+ context_src.size(1)
